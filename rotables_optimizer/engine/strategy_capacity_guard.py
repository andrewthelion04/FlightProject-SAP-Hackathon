"""Strategy variant: strong destination capacity protection and moderate buffers."""
...
class CapacityGuardStrategy:
    """
    Prioritizes avoiding destination overflows with high safety margins.
    Keeps reasonable origin reserves; purchases only when clearly below targets.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            ...
            base_first = min(max(3, int(origin_stock.first_class * 0.18)), 60)
            ...
            base_economy = min(max(35, int(origin_stock.economy * 0.28)), 260)
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        low_economy = int(cap_economy * 0.32)
        ...
        if hub_stock.first_class < 220:
            buy_first = min(120, max(0, cap_first - hub_stock.first_class - 30))

        if hub_stock.business_class < 320:
            buy_business = min(170, max(0, cap_business - hub_stock.business_class - 30))

        if hub_stock.premium_economy < 380:
            buy_premium = min(170, max(0, cap_premium - hub_stock.premium_economy - 30))
