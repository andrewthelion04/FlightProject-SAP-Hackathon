"""Strategy variant: prioritizes keeping HUB1 highly stocked; restrained outstations."""
...
class HubPriorityStrategy:
    """
    Maintains high reserves at HUB1 and sends leaner loads from outstations.
    Purchases aggressively if HUB1 dips below targets.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            reserve_first = int(forecast.first_class * 1.7)
            reserve_business = int(forecast.business_class * 1.7)
            reserve_premium = int(forecast.premium_economy * 1.55)
            reserve_economy = int(forecast.economy * 1.55)
        else:
            base_first = min(max(2, int(origin_stock.first_class * 0.12)), 30)
            base_business = min(max(3, int(origin_stock.business_class * 0.12)), 40)
            base_premium = min(max(3, int(origin_stock.premium_economy * 0.12)), 40)
            base_economy = min(max(25, int(origin_stock.economy * 0.22)), 200)

            reserve_first = max(base_first, int(forecast.first_class * 0.9))
            reserve_business = max(base_business, int(forecast.business_class * 0.9))
            reserve_premium = max(base_premium, int(forecast.premium_economy * 0.9))
            reserve_economy = max(base_economy, int(forecast.economy * 0.9))
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        low_economy = int(cap_economy * 0.48)
        high_economy = int(cap_economy * 0.82)

        if hub_stock.economy < low_economy:
            desired = 11000
            free_space = max(0, high_economy - hub_stock.economy)
            buy_economy = min(desired, free_space, 11000)

        if hub_stock.first_class < 280:
            buy_first = min(160, max(0, cap_first - hub_stock.first_class - 10))

        if hub_stock.business_class < 380:
            buy_business = min(200, max(0, cap_business - hub_stock.business_class - 10))

        if hub_stock.premium_economy < 450:
            buy_premium = min(200, max(0, cap_premium - hub_stock.premium_economy - 10))
