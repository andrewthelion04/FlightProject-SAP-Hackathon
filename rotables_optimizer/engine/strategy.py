"""Strategy layer that decides what to load and what to purchase each hour."""
...
class BalancedDispatchStrategy:
    """
    A deliberately verbose, safety-focused strategy that:
    - Protects future demand at the origin (keeps a forecast buffer).
    - Avoids overfilling destination airports by checking capacity slack.
    - Keeps HUB1 supplied using simple guardrails.
    """
    ...
    # ===========================================================
    # Entry point per simulated hour
    # ===========================================================
    def plan_round(self, day: int, hour: int, state: SimulationState) -> RoundInstruction:
        self.current_day = day
        self.current_hour = hour

        # First, move finished processing back into usable inventory.
        self.stock.advance_processing(day, hour)

        load_plans = []
        flights_to_handle = state.pull_pending_loads()

        for flight_event in flights_to_handle:
            ...
            # Immediately deduct from origin to keep inventory consistent.
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            reserve_first = int(forecast.first_class * 1.3)
            reserve_business = int(forecast.business_class * 1.3)
            reserve_premium = int(forecast.premium_economy * 1.2)
            reserve_economy = int(forecast.economy * 1.2)
        else:
            base_first = min(max(3, int(origin_stock.first_class * 0.20)), 50)
            base_business = min(max(5, int(origin_stock.business_class * 0.20)), 80)
            base_premium = min(max(5, int(origin_stock.premium_economy * 0.20)), 80)
            base_economy = min(max(40, int(origin_stock.economy * 0.30)), 300)
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        high_economy = int(cap_economy * 0.70)

        if hub_stock.economy < low_economy:
            desired = 8000
            free_space = max(0, high_economy - hub_stock.economy)
            buy_economy = min(desired, free_space, 8000)

        if hub_stock.first_class < 200:
            buy_first = min(100, max(0, cap_first - hub_stock.first_class - 20))

        if hub_stock.business_class < 300:
            buy_business = min(150, max(0, cap_business - hub_stock.business_class - 30))

        if hub_stock.premium_economy < 400:
            buy_premium = min(150, max(0, cap_premium - hub_stock.premium_economy - 30))
