"""Strategy variant: conservative buffers and large destination safety margins."""
...
class ConservativeStrategy:
    """
    Keeps larger reserves and destination slack; slower to ship kits out.
    Purchases are modest to avoid overfilling HUB1.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            reserve_first = int(forecast.first_class * 1.5)
            reserve_business = int(forecast.business_class * 1.5)
            reserve_premium = int(forecast.premium_economy * 1.35)
            reserve_economy = int(forecast.economy * 1.35)
        else:
            base_first = min(max(5, int(origin_stock.first_class * 0.25)), 80)
            base_business = min(max(7, int(origin_stock.business_class * 0.25)), 100)
            base_premium = min(max(7, int(origin_stock.premium_economy * 0.25)), 100)
            base_economy = min(max(60, int(origin_stock.economy * 0.35)), 400)

            reserve_first = max(base_first, int(forecast.first_class * 1.25))
            reserve_business = max(base_business, int(forecast.business_class * 1.25))
            reserve_premium = max(base_premium, int(forecast.premium_economy * 1.25))
            reserve_economy = max(base_economy, int(forecast.economy * 1.25))
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        low_economy = int(cap_economy * 0.42)
        high_economy = int(cap_economy * 0.78)

        if hub_stock.economy < low_economy:
            desired = 6500
            free_space = max(0, high_economy - hub_stock.economy)
            buy_economy = min(desired, free_space, 6500)

        if hub_stock.first_class < 300:
            buy_first = min(80, max(0, cap_first - hub_stock.first_class - 40))

        if hub_stock.business_class < 400:
            buy_business = min(120, max(0, cap_business - hub_stock.business_class - 50))

        if hub_stock.premium_economy < 450:
            buy_premium = min(120, max(0, cap_premium - hub_stock.premium_economy - 50))
