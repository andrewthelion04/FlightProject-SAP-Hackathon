"""Strategy variant: aggressive loading with minimal buffers and fast purchases."""
...
class AggressiveStrategy:
    """
    Pushes more kits out by holding smaller reserves and smaller destination margins.
    Purchases early to keep HUB1 fed.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            ...
            base_first = min(max(2, int(origin_stock.first_class * 0.12)), 35)
            base_business = min(max(3, int(origin_stock.business_class * 0.12)), 45)
            base_premium = min(max(3, int(origin_stock.premium_economy * 0.12)), 45)
            base_economy = min(max(25, int(origin_stock.economy * 0.22)), 220)
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        low_economy = int(cap_economy * 0.28)
        high_economy = int(cap_economy * 0.62)

        if hub_stock.economy < low_economy:
            desired = 9000
            free_space = max(0, high_economy - hub_stock.economy)
            buy_economy = min(desired, free_space, 9000)

        if hub_stock.first_class < 240:
            buy_first = min(140, max(0, cap_first - hub_stock.first_class - 12))

        if hub_stock.business_class < 340:
            buy_business = min(180, max(0, cap_business - hub_stock.business_class - 12))

        if hub_stock.premium_economy < 440:
            buy_premium = min(180, max(0, cap_premium - hub_stock.premium_economy - 12))
