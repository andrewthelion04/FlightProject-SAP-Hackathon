"""Strategy variant: prioritizes economy stock with higher thresholds and buys."""
...
class HighEconomyStrategy:
    """
    Keeps larger economy buffers and buys aggressively when low.
    Other cabins follow balanced rules with standard margins.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            reserve_first = int(forecast.first_class * 1.25)
            reserve_business = int(forecast.business_class * 1.25)
            reserve_premium = int(forecast.premium_economy * 1.15)
            reserve_economy = int(forecast.economy * 1.5)
        else:
            base_first = min(max(3, int(origin_stock.first_class * 0.18)), 50)
            base_business = min(max(4, int(origin_stock.business_class * 0.18)), 70)
            base_premium = min(max(4, int(origin_stock.premium_economy * 0.18)), 70)
            base_economy = min(max(75, int(origin_stock.economy * 0.42)), 480)

            reserve_first = max(base_first, int(forecast.first_class * 1.05))
            reserve_business = max(base_business, int(forecast.business_class * 1.05))
            reserve_premium = max(base_premium, int(forecast.premium_economy * 1.05))
            reserve_economy = max(base_economy, int(forecast.economy * 1.35))
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        if hub_stock.business_class < 300:
            buy_business = min(150, max(0, cap_business - hub_stock.business_class - 20))

        if hub_stock.premium_economy < 380:
            buy_premium = min(150, max(0, cap_premium - hub_stock.premium_economy - 20))
