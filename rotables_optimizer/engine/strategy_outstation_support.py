"""Strategy variant: favors keeping outstations healthier with larger local buffers."""
...
class OutstationSupportStrategy:
    """
    Holds slightly smaller HUB1 reserves but protects outstations with higher minima.
    Purchases are moderate; loads aim to avoid depleting remote stocks too fast.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            ...
            reserve_economy = int(forecast.economy * 1.15)
        else:
            base_first = min(max(5, int(origin_stock.first_class * 0.28)), 85)
            base_business = min(max(7, int(origin_stock.business_class * 0.28)), 105)
            base_premium = min(max(7, int(origin_stock.premium_economy * 0.28)), 105)
            base_economy = min(max(65, int(origin_stock.economy * 0.38)), 400)

            reserve_first = max(base_first, int(forecast.first_class * 1.2))
            reserve_business = max(base_business, int(forecast.business_class * 1.2))
            reserve_premium = max(base_premium, int(forecast.premium_economy * 1.2))
            reserve_economy = max(base_economy, int(forecast.economy * 1.2))
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        high_economy = int(cap_economy * 0.65)

        if hub_stock.economy < low_economy:
            desired = 7000
            free_space = max(0, high_economy - hub_stock.economy)
            buy_economy = min(desired, free_space, 7000)

        if hub_stock.first_class < 180:
            buy_first = min(120, max(0, cap_first - hub_stock.first_class - 30))
