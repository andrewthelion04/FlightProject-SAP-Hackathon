"""Strategy variant: never purchases kits, only redistributes existing stock."""
...
class NoPurchaseStrategy:
    """
    Moves kits around without issuing procurement orders.
    Keeps modest buffers at origins and respects destination capacity using if-based guards.
    """
    ...
    def plan_round(self, day: int, hour: int, state: SimulationState) -> RoundInstruction:
        ...
        procurement = CabinKits()  # never buy
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            reserve_first = int(forecast.first_class * 1.15)
            reserve_business = int(forecast.business_class * 1.15)
            reserve_premium = int(forecast.premium_economy * 1.1)
            reserve_economy = int(forecast.economy * 1.1)
        else:
            base_first = min(max(3, int(origin_stock.first_class * 0.18)), 45)
            base_business = min(max(4, int(origin_stock.business_class * 0.18)), 65)
            base_premium = min(max(4, int(origin_stock.premium_economy * 0.18)), 65)
            base_economy = min(max(40, int(origin_stock.economy * 0.28)), 260)
