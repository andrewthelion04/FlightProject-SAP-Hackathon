"""Strategy variant: keeps minimal buffers and smaller purchase quantities."""
...
class LeanBuffersStrategy:
    """
    Minimizes held inventory to reduce idle stock.
    Uses tighter destination margins and lower procurement volumes.
    """
    ...
    def _build_load_plan(self, evt, state: SimulationState) -> FlightLoadPlan:
        ...
        if origin == "HUB1":
            reserve_first = int(forecast.first_class * 1.08)
            reserve_business = int(forecast.business_class * 1.08)
            reserve_premium = int(forecast.premium_economy * 1.02)
            reserve_economy = int(forecast.economy * 1.02)
        else:
            base_first = min(max(2, int(origin_stock.first_class * 0.14)), 28)
            base_business = min(max(3, int(origin_stock.business_class * 0.14)), 38)
            base_premium = min(max(3, int(origin_stock.premium_economy * 0.14)), 38)
            base_economy = min(max(18, int(origin_stock.economy * 0.20)), 190)
    ...
    def _decide_procurement(self) -> CabinKits:
        ...
        low_economy = int(cap_economy * 0.24)
        ...
        if hub_stock.first_class < 150:
            buy_first = min(80, max(0, cap_first - hub_stock.first_class - 10))

        if hub_stock.business_class < 230:
            buy_business = min(100, max(0, cap_business - hub_stock.business_class - 10))

        if hub_stock.premium_economy < 260:
            buy_premium = min(100, max(0, cap_premium - hub_stock.premium_economy - 10))
